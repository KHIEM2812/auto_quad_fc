...
CSTMWinMainForm::CSTMWinMainForm(char *lpzText, CRect rAreaRect, uint16_t nTextColor,  uint16_t nBackColor, sFONT *pFont)
                :CSTMWinForm(INVALID_HANDLE, rAreaRect, nTextColor, nBackColor, pFont)
{
  //init the LCD driver
  CSTMLCDMngr::Init(); 
  ...
  m_CountPage = 0;
  m_ActivePage = 0;
  m_TitleBar = CSTMWinForm(this, CRect(0, 0, rAreaRect.Width(), Font16x24.Height + 2), White, Blue, &Font16x24);
  m_TitleBar.InvalidateArea();
  m_StatusBar = CSTMWinForm(this, CRect(0, rAreaRect.Height()-(pFont->Height + 2), rAreaRect.Width(), pFont->Height + 2), nTextColor, nBackColor, pFont);
  m_StatusBar.InvalidateArea();
  m_WorkArea = CSTMWinForm(this, CRect(0, m_TitleBar.GetHeight(), rAreaRect.Width(), rAreaRect.Height() - m_StatusBar.GetHeight() - m_TitleBar.GetHeight()), Black, White, pFont);
