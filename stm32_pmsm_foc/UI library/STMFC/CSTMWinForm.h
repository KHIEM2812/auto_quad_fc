...
public:
  ...
  uint8_t KeyPress(InputKey key);
  uint8_t KeyPressed(InputKey key);
  uint8_t KeyReleased(InputKey key);
...
public:
  ...
  void Refresh(){/* if (m_CountPage > m_ActivePage && GetActivePage() != INVALID_HANDLE) GetActivePage()->InvalidateArea();*/};
