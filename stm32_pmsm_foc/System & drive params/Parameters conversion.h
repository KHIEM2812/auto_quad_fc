...
#define LF_TASK_FREQUENCY_HZ        100
...
#define SAFETY_TASK_OCCURENCE_TICKS 0
#define LF_TASK_OCCURENCE_TICKS  (SYS_TICK_FREQUENCY/LF_TASK_FREQUENCY_HZ)-1u
...
#define MOTOR_MAX_SPEED_HZ (s16)((MOTOR_MAX_SPEED_RPM*10u)/60u)
...
#define MOTOR_MAX_SPEED_DPP (int32_t)((1.2*MOTOR_MAX_SPEED_RPM*65536*POLE_PAIR_NUM)\
                                                            /(TF_REGULATION_RATE*60))
...
#define DELTA_TEMP_HYSTERESIS        (OV_TEMPERATURE_HYSTERESIS_C- T0_C)
#define DELTA_V_HYSTERESIS           (dV_dT * DELTA_TEMP_HYSTERESIS)
#define OV_TEMPERATURE_HYSTERESIS_d  ((V0_V + DELTA_V_HYSTERESIS)*INT_SUPPLY_VOLTAGE)
...
#if (LOW_SIDE_SIGNALS_ENABLING == ENABLE)
#define DEADTIME_NS  (uint16_t)SW_DEADTIME_NS
#else
#define DEADTIME_NS  HW_DEAD_TIME_NS
#endif

#define DEAD_TIME_COUNTS  (uint16_t)(ADV_TIM_CLK_MHz * DEADTIME_NS/1000uL)
...
#if LCD_JOYSTICK_BUTTON_FUNCTIONALITY == ENABLE
...
#define UI_CONFIG_M1 ( UI_CFGOPT_NONE DAC_OP_ENABLE FW_ENABLE DIFFTERM_ENABLE \
  | (MAIN_SCFG << MAIN_SCFG_POS) | (AUX_SCFG << AUX_SCFG_POS) | UI_CFGOPT_SETIDINSPDMODE PLLTUNING_ENABLE | UI_CFGOPT_PLLTUNING)
