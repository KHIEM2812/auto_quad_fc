...
#define CHARGE_BOOT_CAP_TICKS  (uint16_t)((SYS_TICK_FREQUENCY * CHARGE_BOOT_CAP_MS)/ 1000)   // or WB
#define CHARGE_BOOT_CAP_TICKS2 (uint16_t)((SYS_TICK_FREQUENCY * CHARGE_BOOT_CAP_MS2)/ 1000)  // or WB
#define OFFCALIBRWAITTICKS     (uint16_t)((SYS_TICK_FREQUENCY * OFFCALIBRWAIT_MS)/ 1000)     // or WB
#define OFFCALIBRWAITTICKS2    (uint16_t)((SYS_TICK_FREQUENCY * OFFCALIBRWAIT_MS2)/ 1000)    // or WB
#define STOPPERMANENCY_TICKS   (uint16_t)((SYS_TICK_FREQUENCY * STOPPERMANENCY_MS)/ 1000)    // or WB
#define STOPPERMANENCY_TICKS2  (uint16_t)((SYS_TICK_FREQUENCY * STOPPERMANENCY_MS2)/ 1000)   // or WB
...
#error "Invalid boot: configuration not valid or to be assigned"
  ...
  RUC_Init(oRUC_M1,oSTC[M1],(CVSS_SPD)oVSS_M1);        /* only if sensorless*/
#endif
#if ((defined ENCODER)||(defined VIEW_ENCODER_FEEDBACK))
...
#error "Invalid boot: configuration not valid or to be assigned"
  ...
  RUC_Init(oRUC_M2,oSTC[M2],(CVSS_SPD)oVSS_M2);        /* only if sensorless*/
#endif
#if ((defined ENCODER2)||(defined VIEW_ENCODER_FEEDBACK2))  
    ...
    {    
      ...
      if (StartUpTransitionEnded == TRUE)
      {
        {      
          ...
          StatorCurrent = FOCVars[M1].Ialphabeta;
          StatorCurrent = MCM_Park(StatorCurrent, SPD_GetElAngle(oSpeedSensor[M1]));
          PI_SetIntegralTerm(oPIDSpeed[M1],(int32_t)StatorCurrent.qI_Component1*PI_GetKIDivisor(oPIDSpeed[M1])); 
        }        
  ...
  case START:
    ...
    {     
      ...
      if (StartUpTransitionEnded == TRUE)
      {
        {      
          Curr_Components StatorCurrent;
          StatorCurrent = FOCVars[M2].Ialphabeta;
          StatorCurrent = MCM_Park(StatorCurrent, SPD_GetElAngle(oSpeedSensor[M2]));
          PI_SetIntegralTerm(oPIDSpeed[M2],(int32_t)StatorCurrent.qI_Component1*PI_GetKIDivisor(oPIDSpeed[M2])); 
        }        
...
{ 
  ...
  hFOCreturn = FOC_CurrController(bMotorNbr);
